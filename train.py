...
def train(args):
    ...
    idim = hp.symbol_len
    odim = hp.num_mels
    model = transformer.Transformer(idim, odim, args)
    # set torch device
    ...
    for epoch in range(hp.epochs):
        ...
        for data in pbar:
            #start_b = time.time()
            global_step += 1
            x, input_length, y, stop_token, out_length = data
            ...
            loss, report_dict = model(x.cuda(), input_length.cuda(), y.cuda(), stop_token.cuda(), out_length.cuda())
            ...
            if step % hp.validation_step == 0:
                ...
                for valid in validloader:
                    x_, input_length_, y_, stop_token_, out_length_ = valid
                    model.eval()
                    loss_, report_dict_ = model(x_.cuda(), input_length_.cuda(), y_.cuda(), stop_token_.cuda(), out_length_.cuda())
...
def main(cmd_args):
    ...
    transformer.Transformer.add_arguments(parser)
